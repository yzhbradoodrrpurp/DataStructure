...
定义：二叉排序树也称为二叉查找树 (BST)，二叉排序树或者是一棵空树；或者是具有如下特性的二叉树，
...
- 如果 p 是叶节点，则直接删除
- 如果 p 只有左/右子树，则删除 p 之后将 p 的子树直接当做 p 父节点的子树
  - 如果 p 是左子树，则 p 的子树当做 p 的父节点的左子树
  - 如果 p 是右子树，则 p 的子树当做 p 的父节点的右子树

- 如果 p 既有左子树也有右子树，找到在中序序列中 p 的前驱节点 s 并将 s 的值复制到 p，然后依照同样的方式删除 s 节点
