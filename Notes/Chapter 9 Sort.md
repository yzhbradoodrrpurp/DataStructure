...
| 排序方法名称 | 最好情况时间复杂度 | 最坏情况时间复杂度 | 平均时间复杂度              | 稳定性 | 辅助存储空间 |
| :----------- | :----------------- | ------------------ | --------------------------- | ------ | ------------ |
| 直接插入排序 | $O(n)$             | $O(n^2)$           | $O(n^2)$                    | 稳定   | $O(1)$       |
| 希尔排序     | $O(n)$             | $O(n^2)$           | $O(n^{1.3})$ ~ $O(n^{1.5})$ | 不稳定 | $O(1)$       |
| 冒泡排序     | $O(n)$             | $O(n^2)$           | $O(n^2)$                    | 稳定   | $O(1)$       |
| 快速排序     | $O(n\log_2 n)$     | **$O(n^2)$**       | $O(n\log_2 n)$              | 不稳定 | $O(\log_2n)$ |
| 简单选择排序 | $O(n^2)$           | $O(n^2)$           | $O(n^2)$                    | 不稳定 | $O(1)$       |
| 堆排序       | $O(n\log_2n)$      | **$O(n\log_2n)$**  | $O(n\log_2n)$               | 不稳定 | $O(1)$       |
| 归并排序     | $O(n\log_2n)$      | $O(n\log_2n)$      | $O(n\log_2n)$               | 稳定   | $O(n)$       |
| 基数排序     | -                  | -                  | -                           | 稳定   | -            |

- 希尔排序、快速排序和所有的选择排序（简单选择排序、堆排序）是不稳定的，其它排序方式都是稳定的。

- 虽然快速排序是原地排序，但是会占用辅助存储空间，因为快速排序会进行递归调用。辅助存储空间最好情况下为 $O(\log_2n)$，最坏情况下为 $O(n)$，平均情况下为 $O(\log_zn)$。

- 归并排序的辅助存储空间为 $O(n)$，因为在合并过程中，我们需要比较两个子序列的元素。如果你直接在原数组上操作，当把一个较小的元素挪到前面时，可能会覆盖掉原数组中还没来得及比较的数据。为了保证数据不丢失，标准的做法是先开辟一个与原数组等长的**辅助数组**。
