...
以 [row, col, value] 的方式存储矩阵。在存储稀疏矩阵时，可以有效地节省空间。

- 用三元组存储稀疏矩阵时，通常第一个三元组是 [矩阵行数, 矩阵列数, 非空元素个数]
- 有时用单独的成员变量来封装矩阵的行数、矩阵的列数、非空元素个数这三个变量

### 十字链表

- 用 right 指针指向同一行中的下一个非空元素
- 用 down 指针指向同一列中的下一个非空元素
