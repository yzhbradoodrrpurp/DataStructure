...
- 节点的深度：从根节点开始往下数到该节点的层数
- 节点的高度：**从该节点的子树中最远的叶节点往上数到该节点的层数**
...
- 给定一个树，有唯一的二叉树与之对应
  - 树的左指针指向第一个子树 (如果左子树存在即指向左子树，不存在即指向右子树)
  - 树的右指针指向右边第一个兄弟树

- **在森林和二叉树的转换中，森林中树的个数等于二叉树中最右边线路上节点的个数**
- **在森林和二叉树的转换中，森林中叶子的个数等于对应二叉树中左指针为空的节点数**
...
- 对有 n 个节点的完全二叉树按层次遍历并进行标号，则标号为 i 的节点满足如下规则：
  - 如果 $i = 1$，则是根节点
  - **如果 $i > n // 2$，则是叶节点**
  - **如果 $i < n // 2$，则是非叶节点**
  - **$2i + 1$ 是 i 的左子节点索引，$2i + 2$ 是 i 的右子节点索引（如果左子结点或右子结点存在）**
...
0 个节点可以构成 0 个不同的树 $f(0) =0$，

1 个节点可以构成 1 个不同的树 $f(1) = 1$，

2 个节点可以构成不同的树 $f(2) = f(1) \cdot f(0) + f(0) \cdot f(1)$，

3 个节点可以构成不同的树 $f(3) = f(2) \cdot f(0) + f(1) \cdot f(1) + f(0) \cdot f(2)$，

...

**$n$ 个节点构成不同的树 $f(n) = \frac{(2n)!}{(n+1)! \cdot n!}$。**
...
- **有 n 个节点的线索二叉树中有 n+1 个线索。**n 个节点的二叉树中总共有 n-1 条边，2n 个左右索引，则有 n+1 个线索。
...
- **在哈夫曼编码中，任意一个编码都不是其它编码的前缀**
- 哈夫曼编码中，约定左子树的编码为 0，右子树的编码为 1，从根节点到叶节点路径上分支字符组成的字符串作为该叶子节点的编码
