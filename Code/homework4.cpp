...
void crossRepresentation() {
    ...
    for (int idx = 0; idx < size; idx++) {
        ...
        std::cout << "(" << node->getColumn() << ", " << node->getRow() << "): " << node->getValue();

        if (node->getRight())
            std::cout << "\t" << "(" << node->getRight()->getRow() << ", " << node->getRight()->getColumn() << "): " << node->getRight()->getValue() << std::endl;
        if (node->getDown())
            std::cout << "(" << node->getDown()->getRow() << ", " << node->getDown()->getColumn() << "): " << node->getDown()->getValue() << std::endl;
